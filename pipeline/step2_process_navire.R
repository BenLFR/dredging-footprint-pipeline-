...
land_mask_path   <- Sys.getenv("LAND_MASK_FILE", unset = file.path(config_dir, "land_mask", "land_polygons.shp"))
...
# (2) Explicit contoion immediately after merge
...
if (!"Service_speed" %chin% names(dt_nav) || all(is.na(dt_nav$Service_speed))) {
  warning("No speed of service for ", navire_name)
}
...
if ("Service_speed" %chin% names(dt_nav)) {
  ...
  n_after  <- nrow(dt_nav)
  cat(sprintf(" Filter speed physique : %d → %d lines (%.2f %% conservees)\n",
...
repeat {
  ...
  # Exit early when no new points are flagged
  if (n_flagged == 0) {
  ...
  cat(sprintf(" → %d points removed (total cumule: %d)\n", n_flagged, total_removed))
...
cat(sprintf(" Filters geospatial : %d lines removed en %d iteration(s) (%.2f %%)\n",
...
cat(" Parameters IF: contamination =", outlier_config$contamination_rate, 
...
cat("Additional columns created (Year, Course_change, Accel)\n")
