...
# -----------------------------------------------------------------------------
# LECTURE DES SPÉCIFICATIONS NAVIRES (service speed, drag-width, …)
# -----------------------------------------------------------------------------
...
ship_specs <- rbindlist(spec_list, fill = TRUE)   # → data.table
...
if ("Service_speed" %chin% names(dt_nav)) {
  ...
  # (3) Suppression de Service_speed si non utilisée plus loin
  dt_nav[, Service_speed := NULL]
}
...
# Calcul des intervalles temporels (en secondes)
dt_nav[, delta_t := c(NA_real_, diff(as.numeric(Timestamp)))]
...
# Calcul des changements de cap (différence entre observations consécutives)
dt_nav[, Course_change := c(NA_real_, abs(diff(Course)))]
# Normalisation des changements de cap (0-180°)
dt_nav[Course_change > 180, Course_change := 360 - Course_change]

# Calcul de l'accélération (différence de vitesse)
dt_nav[, Accel := c(NA_real_, diff(Speed))]
