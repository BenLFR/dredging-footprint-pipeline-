...
if(nrow(tile_bb) == 0) {
  stop("❌ Tuile", tile_id, "non trouvée dans", tiles_file)
}
...
lines_sf  <- dredge_sf %>%
  group_by(sub_seg_id) %>%
  filter(n()>1) %>%
  summarise(W_v   = first(suction_pipe_diameter_m),
...
grid1km$row <- as.integer(floor((grid_coords[,2] - WORLD_YMIN)/CELL_SIZE_M))
grid1km$global_grid_id <- grid1km$col + grid1km$row*GRID_COLS + 1L
...
if(nrow(res) > 0) {
  ...
  cat("   - SAR moyen :", format(mean(res$sum_dw / CELL_AREA_M2), scientific=FALSE), "m²\n")
  cat("   - SAR max :", format(max(res$sum_dw / CELL_AREA_M2), scientific=FALSE), "m²\n")
}
