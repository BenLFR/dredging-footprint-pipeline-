...
ais <- checkpoint("stage3C_gmm.rds", {
  ...
  ## 5. Étiquettes brutes - CORRECTION: Mapping intelligent basé sur la vitesse ---
  # CORRECTION CRITIQUE: Attribution intelligente des labels selon la plage de vitesse
  # μ mobiles triés : 1.68 / 5.29 / 9.33 / 12.59 kn → mapping intelligent
  
  ## --- Mapping intelligent des composantes ------------------------------ ##
  # Paramètres généraux
  dredge_range <- c(1, 3.5)           # fenêtre (kn) considérée comme dragage
  slow_range   <- c(3.5, 6)           # manœuvres lentes / sous-charge
  
  # Trouve l'index de la composante dont μ ∈ [1 ; 3.5]
  dredge_idx  <- which(mu >= dredge_range[1] & mu <= dredge_range[2])
  
  # Si aucune composante n'est exactement dans la fenêtre, prend la plus proche de 2 kn
  if (length(dredge_idx) == 0) {
      dredge_idx <- which.min(abs(mu - mean(dredge_range)))
  }
  
  # SÉCURISATION: Si plusieurs composantes dans la plage, prendre la plus proche de 2 kn
  if (length(dredge_idx) > 1) {
      dredge_idx <- dredge_idx[which.min(abs(mu[dredge_idx] - 2))]  # la plus proche de 2 kn
  }
  
  # Idem pour « slow_maneuvers » : la composante juste au-dessus de la zone dragage
  slow_idx <- setdiff(which(mu > slow_range[1] & mu <= slow_range[2]), dredge_idx)
  
  # Reste : loaded / unloaded transit triés par vitesse
  transit_idx <- setdiff(seq_along(mu), c(dredge_idx, slow_idx))
  loaded_idx      <- transit_idx[1]                # plus lente des deux
  unloaded_idx    <- transit_idx[length(transit_idx)]
  
  # Construction finale des labels (longueur = K)
  mobile_labels <- character(K)            # initialise
  mobile_labels[dredge_idx]   <- "dredging"
  if (length(slow_idx))    mobile_labels[slow_idx]    <- "slow_maneuvers"
  mobile_labels[loaded_idx]  <- "loaded_transit"
  if (unloaded_idx != loaded_idx)
      mobile_labels[unloaded_idx] <- "unloaded_transit"
  
  # (optionnel) toute composante non étiquetée héritera de "other"
  ...
  cat("   🎯 Composante dragage: μ =", round(mu[dredge_idx], 2), "kn (index", dredge_idx, ")\n")
  ...
  # 2) Vector logique sans NA pour les tests ultérieurs
  dredge_points <- ais$behavior_smooth == "dredging"
  # (inutile désormais d'ajouter la ligne "dredge_points[is.na…] <- FALSE")
  ## ───────────────────────────────────────────────────
