...
# Runtime on cluster (emlab_nodes, 16G, 1 CPUs): ~15–30 minutes.
#
# Outputs:
#   output_V6/sensitivity/sobol_indices.csv  — S1 and ST per parameter
#   output_V6/sensitivity/sobol_plot.png     — bar chart (300 dpi)
#
# Usage:
#   Rscript --vanilla analysis/sensitivity_sobol.R
#
# On SLURM (single job — NOT an array):
#   sbatch --ntasks=1 --cpus-per-task=4 --mem=16G --time=1:00:00 \
#          --wrap="Rscript --vanilla analysis/sensitivity_sobol.R"
#
...
# Source: configuration/fi_parameters_with_freshness.yaml (or constants.R defaults)
fi_defaults <- list(
  alpha_dep           = 0.25,
  fast_fraction       = 0.30,
  slow_k              = 0.05,
...
# Mean k_fast across ocean provinces (from fi_parameters_with_freshness.yaml defaults)
