...
if (is_dry) {
  cat(" === MODE DRY-RUN ACTIVE ===\n")
  cat(" Test on echantillon reduit - Garde-fous assouplis\n")
}

cat(" === STEP 3: FUSION ET GRID SEARCH (WITH CHECK-POINTING) ===\n")
cat(" Job fractionnement:", split_job_id, "\n")
cat(" Directory results:", results_dir, "\n")
cat(" Cores:", cores, "\n")
...
if (file.exists(gmm_checkpoint)) {
  cat(" Suppression of checkpoint GMM fautif for forcer the recalcul\n")
  unlink(gmm_checkpoint)
}

if (file.exists(dbscan_checkpoint)) {
  cat(" Suppression of checkpoint DBSCAN fautif for forcer the recalcul\n")
...
if (results_dir != "" && dir.exists(results_dir)) {
  ...
  cat(" Utilisation of directory specifie:", expected_dir, "\n")
} else {
  cat(" Auto-detection of directory of results...\n")
  ...
  for (path in possible_paths) {
    if (dir.exists(path)) {
      expected_dir <- path
      cat(" Directory found:", expected_dir, "\n")
      break
    } else {
      cat(" Not found:", path, "\n")
    }
  ...
  if (is.null(expected_dir)) {
    stop(" ERROR: No directory of results found")
  }
}
...
if (length(clean_files) == 0) {
  stop(" ERROR: No file *_clean.rds found in ", expected_dir)
}

cat(" Files d'entree verifies:", length(clean_files), "files *_clean.rds found\n\n")

cat("\n STEP-3 | Fusion & Modelisation | start :", format(Sys.time()), "\n\n")
...
cat(" R cherchera the packages in :", paste(.libPaths(), collapse = " | "), "\n")

# DIAGNOSTIC THE PACKAGES - Check that all required packages are available
cat("\n DIAGNOSTIC THE PACKAGES REQUIRED:\n")
...
if (!length(clean_files_check)) {
  stop(" No file *_clean.rds found in ", split_dir, "\n",
       "   Files present in the directory:\n",
       paste("   -", list.files(split_dir, pattern = "\\.rds$"), collapse = "\n"),
       "\n   Check that Step 2 completed successfully.")
}

cat(" Files *_clean.rds found:", length(clean_files_check), "\n")
for (f in head(clean_files_check, 3)) cat(" •", basename(f), "\n")
if (length(clean_files_check) > 3) cat(" ... et", length(clean_files_check) - 3, "other\n")
...
cat(" Packages loaded |", cores, "threads data.table\n")
...
check_filter <- function(step_name, n_before, n_after, 
  ...
  cat(sprintf("[DIAGNOSTIC] %s : %d points removed (%.2f%%), %d restants\n", 
              step_name, n_removed, 100*frac_removed, n_after))
  
  if (fatal) {
    if (frac_removed > max_frac) {
      stop(sprintf("ERROR FATALE : %s a removed %.1f%% the points (>%.0f%%). Check vos parameters ou data !", 
                   step_name, 100*frac_removed, 100*max_frac))
    }
    if (n_after < min_left) {
      stop(sprintf("ERROR FATALE : Il ne reste que %d points after '%s' (<%d), pipeline arretee.", 
                   n_after, step_name, min_left))
    }
  } else {
    if (frac_removed > max_frac) {
      warning(sprintf("[ALERTE] %s a removed %.1f%% the points (>%.0f%%) !", 
                   step_name, 100*frac_removed, 100*max_frac))
    }
    if (n_after < min_left) {
      warning(sprintf("[ALERTE] Il ne reste que %d points after '%s' (<%d) !", 
                   n_after, step_name, min_left))
    }
  }
}
...
# Lecture of YAML with gestion the errors
tryCatch({
  cfg <- yaml::read_yaml(Sys.getenv("OUTLIER_CONFIG_FILE", unset = file.path(config_dir, "outlier_config_V6.yaml")))
  ...
  for (param in names(default_config)) {
    if (is.null(cfg[[param]])) {
      ...
      cat(" Parameter missing in YAML:", param, "- Utilisation valeur par default:", default_config[[param]], "\n")
    }
  }
}, error = function(e) {
  cat(" Error lecture YAML - Utilisation valeurs par default\n")
  cfg <<- default_config
})
...
rm(ais_list); gc()
cat(sprintf(" Fusion : %s lines | %.1f s\n\n",
...
# CRITICAL CHECK: abort if rows without Year are present after merge
nb_na_year <- ais[is.na(Annee), .N]
if (nb_na_year > 0) {
  stop(nb_na_year, " lines without Year after fusion – checkpoint not save.")
}
...
if (is_dry && DRYRUN_N > 0 && nrow(ais) > DRYRUN_N) {
  ...
  cat(sprintf(" Dry-run : subset of %d lines (%.1f %% of total initial)\n",
...
# --------------------------------------------------------------------------
cat(" Verification the types of columns...\n")

# Critical type verification
cat(" Types the columns critiques:\n")
...
if ("Navire" %in% names(ais)) {
  ...
  if (!is.character(ais$Navire)) {
    cat(" Contoion of Vessel en caractere\n")
    ais[, Navire := as.character(Navire)]
  }
}

if ("ssvid" %in% names(ais)) {
  # S'assurer que ssvid is of type entier
  if (!is.integer(ais$ssvid) && !is.numeric(ais$ssvid)) {
    cat(" Contoion of ssvid en entier\n")
...
if ("Seg_id" %in% names(ais)) {
  ...
  if (!is.character(ais$Seg_id)) {
    cat(" Contoion of Seg_id en caractere\n")
    ais[, Seg_id := as.character(Seg_id)]
  }
}

if ("Annee" %in% names(ais)) {
  # S'assurer que Year is of type entier
  if (!is.integer(ais$Annee) && !is.numeric(ais$Annee)) {
    cat(" Contoion of Year en entier\n")
    ais[, Annee := as.integer(Annee)]
  }
}

# ADDITIONAL CRITICAL CHECKS
cat(" Verifications supplementaires critiques...\n")

# 1. Verify is_stop column (required by DBSCAN)
if (!"is_stop" %chin% names(ais)) {
  stop(" Column 'is_stop' missing - Check que the Step-2 s'is bien completed")
} else {
  cat(" Column 'is_stop' present\n")
}
...
if (length(missing_geo) > 0) {
  stop(" Columns geographic missing: ", paste(missing_geo, collapse = ", "))
} else {
  cat(" Columns geographic present\n")
  # Convert to numeric to avoid type errors
  ais[, c("Lon", "Lat", "Course", "Speed") := lapply(.SD, as.numeric), .SDcols = c("Lon", "Lat", "Course", "Speed")]
  cat(" Columns geographic converties en numeric\n")
}

# 3. Check for duplicate (Seg_id, Timestamp)
if (anyDuplicated(ais, by = c("Seg_id", "Timestamp"))) {
  warning(" Doublons detectes in (Seg_id, Timestamp) - Suppression...")
  ais <- unique(ais, by = c("Seg_id", "Timestamp"))
  cat(" Doublons removed\n")
} else {
  cat(" No doublon detected\n")
}

# 4. Check available memory
mem_usage <- object.size(ais) / 1024^3  # GB
cat(sprintf(" Utilisation memory: %.2f GB\n", mem_usage))

if (mem_usage > 50) {
  warning(" Utilisation memory elevee: ", round(mem_usage, 1), " GB")
}

cat(" Types of columns verifies et corriges\n\n")

# --------------------------------------------------------------------------
# STRICT ROW-COUNT CONTROL - ABORT ON MISMATCH
# --------------------------------------------------------------------------
n_total <- nrow(ais)  # Effectif initial
cat(" === CONTROLE STRICT THE EFFECTIFS ===\n")
cat(sprintf(" Nombre of points to start: %d\n", n_total))

# --------------------------------------------------------------------------
# 5. ── SUPPRIMER DIRECTEMENT THE outliers_IF -------------------------------
# --------------------------------------------------------------------------
...
if ("outlier_IF" %chin% names(ais)) {
  ...
  cat("[DIAGNOSTIC] Isolation Forest: No filter applique (column absente)\n")
}

# --------------------------------------------------------------------------
# 6. ── FONCTIONS UTILITAIRES (reprise of V6 complet) -----------------------
# --------------------------------------------------------------------------
...
detect_spikes <- function(speed, spike_factor = 5, min_iso = 2) {
  ...
  cand              # retour implicite (plus of `return()`)
}
...
# Smoothing transition comportement (idem Step-2 script)
...
# --------------------------------------------------------------------------
# 7. ── PICS TEMPORELS (Step-4 of pipeline) -------------------------------
# --------------------------------------------------------------------------
cat(" Detection pics temporels isoles...\n")
...
cat(" Detection the arrets spatiaux par DBSCAN...\n")

# OPTIMISATION: Protection contre the petits vessels et approche plus robuste
...
stop_counts <- stop_counts[N >= 4]  # Only the navires with suffisamment of points

if (nrow(stop_counts) > 0) {
  cat(" Traitement DBSCAN for", nrow(stop_counts), "combinaisons vessel/year\n")
  ...
  for (i in 1:nrow(stop_counts)) {
    ...
    if (length(idx) >= 4) {
      # PROTECTION: Check of the variance the coordata
      if (var(arr$Lon, na.rm = TRUE) < 1e-8 || var(arr$Lat, na.rm = TRUE) < 1e-8) {
        cat(" insufficient variance for", navire, annee, "- Skipping\n")
        next
      }
      ...
      tryCatch({
        ...
        if (nrow(xy) > 5000) {
          cat(" large stop detected for", navire, annee, "(", nrow(xy), "points) - sampling for eps\n")
          set.seed(123)
        ...
        # Protection contre the clusters vides
        if (sum(cl > 0) == 0) {
          cat(" No cluster found for", navire, annee, "- widening eps radius\n")
          eps <- eps * 1.5
          cl <- dbscan(xy, eps = eps, minPts = 4)$cluster
        }
        ...
        cat(" Error DBSCAN for", navire, annee, ":", e$message, "\n")
      })
    }
  }
} else {
  cat(" No vessel with suffisamment of points d'stop for DBSCAN\n")
}

ais[, is_stop_spatial := is_stop & !is.na(stop_cluster) & stop_cluster > 0]
  
  cat(" DBSCAN completed - Checkpoint save\n")
  
  # CHECK: Row counts after DBSCAN
  n_stop_spatial <- sum(ais$is_stop_spatial, na.rm = TRUE)
  cat(sprintf("[DIAGNOSTIC] After DBSCAN (is_stop_spatial TRUE): %d points (%.2f%% of total)\n", 
              n_stop_spatial, 100 * n_stop_spatial / nrow(ais)))
  
  # Check that DBSCAN did not remove too many points
  if (n_stop_spatial < 1000) {
    warning("[ALERTE] Tres peu of points d'stop spatiaux detectes par DBSCAN (< 1000)")
  }
  
  # CRITICAL CHECK: Verify that is_stop_spatial column was created
  if (!"is_stop_spatial" %in% names(ais)) {
    stop(" ERROR CRITIQUE: Column 'is_stop_spatial' missing after DBSCAN")
  }
  cat(" Verification: Column 'is_stop_spatial' present\n")
  
  ais
})

# --------------------------------------------------------------------------
# 10 bis. ── Calcule the cadence AIS & thresholds adaptives -------------------
# --------------------------------------------------------------------------
...
# Gestion of wrap-around for Course_change
ais[Course_change > 180, Course_change := 360 - Course_change]
# Save robuste si too of NA
ais[is.na(Course_change), Course_change := 0]

# 3) Calcul the statistiques of cadence par vessel
...
# 5) Calcul the thresholds adaptives
# - Threshold of base : 3 × median (captures true stops)
# - Floor: 3 min for highly active vessels
# - Plafond : 900 s (15 min) for the vessels normaux
...
# 8) Calcul of n_min adaptive for the smoothing run-length
# t_threshold = 30 s ~ max duration of a micro-glitch
t_threshold   <- 30                     # 30 s ~ max duration of a micro-glitch
t_seuil <- t_threshold                  # alias conserve for compatibilite historique
n_max_cap <- 8                      # plafond for rester rapide
cadence[, n_min := pmin(n_max_cap, pmax(3, ceiling(t_seuil / p50_dt)))]

# 9) Join with l'ensemble principal
...
cat(" Cadence AIS & seuils adaptatifs par vessel\n")
print(cadence[order(p50_dt), .(Navire, p50_dt, seuil_adaptatif, n_min, prop_long)])

# Filtrage the intervals too longs selon the threshold adaptive of chaque vessel
...
# Contoion Speed en m/s for calculer Accel en m/s2
# FIX 2: Garde-fou contre the vecteurs vides
ais[, Accel := {
  dt <- dt_sec[-1L]
  if (length(dt) == 0) {
    rep(NA_real_, .N)
...
cat(sprintf("[DIAGNOSTIC] Nombre of points conserves avant GMM: %d (%.2f%% of total initial)\n", 
            nrow(ais), 100 * nrow(ais) / n_total))
...
if (is_dry) {
  ...
  if (nrow(ais) < 5000) {
    stop(sprintf("ERROR FATALE : Il ne reste que %d points avant GMM (< 5000), echantillon too small for the dry-run.", nrow(ais)))
  }
} else {
  if (nrow(ais) < 20000) {
    stop(sprintf("ERROR FATALE : Il ne reste que %d points avant GMM (< 20000), pipeline arretee.", nrow(ais)))
  }
}

cat("\n[DIAGNOSTIC] Distribution of dataset juste avant the GMM :\n")
...
ais <- checkpoint("stage3C_gmm.rds", {
  ...
  cat(" GMM « stop + K mobiles » – selection K par BIC (3-5) - ROBUSTE\n")
  ...
  cat(" Quantiles recalcules after filters - Accel 95%:", round(q95_acc, 4), "| Course 95%:", round(q95_turn, 2), "°\n")
  ...
  # Threshold adaptive selon the mode (dry-run vs production)
  min_mobile <- if (is_dry) 50 else 200
  if (nrow(X) < min_mobile)
    stop(" Pas assez of points mobiles complets for ajuster the GMM (", nrow(X), " < ", min_mobile, ")")

  ## 1. Recherche of meilleur K (BIC) - ROBUSTE ------------------------------
  # CORRECTION 5: Protection contre l'overflow memory Mclust
  samp <- if (nrow(X) > 1e6) X[sample(nrow(X), 1e6), ] else X
  cat(" Echantillon for BIC:", nrow(samp), "points (on", nrow(X), ")\n")
  
  G_choices <- 3:5                                   # 3 to 5 mobile components
  
  # CORRECTION CRITIQUE: Fonction helper for extraire un scalaire BIC
  ...
  if (all(is.infinite(bic_vals)))
    stop(" Mclust a failed for toutes the valeurs of K - X too grand ou mal conditionne")
  
  K <- G_choices[which.max(bic_vals)]
  
  # FIX 4: Check of the borne basse K >= 3
  if (K < 3) stop(" The GMM doit avoir to moins 3 components mobiles (dredge/transit)")
  
  cat(" K retenu par BIC :", K, "components mobiles\n")

  ## 2. Ajustement definedtif -------------------------------------------------
  # FIX 3: Protection memory for l'ajustement final
  X_fit <- if (nrow(X) > 2e6) X[sample(nrow(X), 2e6), ] else X
  cat(" Echantillon for ajustement final:", nrow(X_fit), "points (on", nrow(X), ")\n")
  ...
  ## 4. Sort by speed ------------------------------------------------------
  ord  <- order(gmm$parameters$mean[1, ])      # tri croissant of Speed
  mu   <- gmm$parameters$mean[1, ord]
  cat(" μ mobiles tries :", paste(round(mu, 2), collapse = " / "), "kn\n")
  ...
  # Garantit que NO transit n'a μ < 6 kn

  ## --- FIX P0: Arowr mobi with Zord (mobi_keep) + max.col() rapide --- ##
  mobi_keep <- mobi[keep]  # aligne exactly the lines with X / Zord
  ...
  # 1) Identifier the components par plage of speed (FIX P1)
  ...
  cat(" components idle (μ < 1 kn):", paste(idle_candidates, collapse=", "), "\n")
  cat(" components lentes (1 ≤ μ < 6 kn):", paste(slow_candidates, collapse=", "), "\n")
  cat(" candidates dredging [1-3.5 kn]:", paste(dredge_candidates, collapse=", "), "\n")
  ...
  if (length(dredge_candidates) > 0) {
    if (length(dredge_candidates) == 1) {
      ...
      # Multiple candidatees: stability score = mean(norm_course) - mean(norm_acc)
      ...
      cat(" Scores stabilite:", paste(round(stability_scores, 3), collapse=", "), "\n")
    }
  } else {
    # Fallback: plus proche of 2 kn parmi toutes the components
    dredge_comp <- which.min(abs(mu - 2))
    cat(" No candidate in [1-3.5 kn], fallback to μ =", round(mu[dredge_comp], 2), "kn\n")
  }

  # 3) The other components lentes (outside dredge) → slow_maneuto (FIX P1)
  other_slow_idx <- setdiff(slow_candidates, dredge_comp)

  # 4) components transit: UNIQUEMENT μ >= 6 kn
  transit_pool <- which(mu >= slow_ceiling)

  cat(" Pool transit (μ >= 6 kn):", paste(transit_pool, collapse=", "), "\n")

  # 5) Construction the labels
  mobile_labels <- character(K)
  mobile_labels[dredge_comp] <- "dredging"

  # FIX P1: idle_candidatees (μ < 1 kn) → "other"
  if (length(idle_candidates) > 0) {
    mobile_labels[idle_candidates] <- "other"
  }

  # slow_maneuto for the lentes outside dredging
  ...
  # else: pas of transit (toutes the components are lentes)
  ...
  if (length(dredge_comp) != 1) {
    stop(sprintf("ERROR MAPPING: dredging not-unique ou absent (dredge_comp = %s)",
                 paste(dredge_comp, collapse=", ")))
  }

  # ASSERTION 1: No transit with μ < 6 kn
  ...
  for (lbl in transit_labels) {
    ...
    if (length(idx) > 0 && any(mu[idx] < slow_ceiling)) {
      stop(sprintf("ERROR MAPPING: %s assigned a μ = %.2f kn (< %.1f kn)\n mu = %s\n labels = %s",
  ...
  cat(" speeds correspondantes:", paste(round(mu, 2), "kn", collapse=" → "), "\n")
  cat(" Composante dredging: μ =", round(mu[dredge_comp], 2), "kn (index", dredge_comp, ")\n")

  ais[, comp     := apply(.SD, 1, which.max), .SDcols = paste0("p", 1:(K + 1))]
  ais[, behavior := map[comp]]

  ## 6. Smoothing run-length - ROBUSTE -----------------------------------------
  ...
  if ("dredging" %in% all_labels) {
    dredge_state <- which(all_labels == "dredging")  # index in all_labels (1..n_labels)
  ...
  # toion robuste of smooth_transition
  ...
  # FIX: Threshold plus stable for the smoothing (75e percentile to lieu of median)
  lissage_threshold <- quantile(ais$n_min, 0.75, na.rm = TRUE)
  cat(" Seuil of smoothing (75e percentile n_min):", round(lissage_threshold, 1), "\n")
  ...
  cat(" GMM ROBUSTE completed –", table(ais$behavior_smooth), "etats after smoothing\n")
  
  # CLASS VERIFICATION AFTER GMM (for dry-run)
  if (is_dry) {
    n_classes <- length(unique(ais$behavior_smooth))
    if (n_classes < 2) {
      stop(sprintf("ERROR FATALE : Echantillon DRY-RUN single-class after GMM (%d classe) – reprenez un autre sous-ensemble", n_classes))
    }
    cat(" Dry-run: ", n_classes, "classes detectees after GMM\n")
  }
  ...
  attr(ais, "gmm_info") <- list(
    K = K,
    dredge_idx = dredge_comp,          # 1..K (index in mobile_labels)
  ...
  if (n_raw_dredge > 0) {
    ...
    if (qa1_rate >= 0.99) {
      cat(sprintf(" QA1 (pre-smooth): %.1f%% the points dredging ont p_dredge=argmax\n", 100*qa1_rate))
    } else if (qa1_rate >= 0.90) {
      cat(sprintf(" QA1 (pre-smooth): %.1f%% only (expected ≥99%%) - check the mapping\n", 100*qa1_rate))
    } else {
      cat(sprintf(" QA1 (pre-smooth): FAILURE - only %.1f%% (expected ≥90%%)\n", 100*qa1_rate))
      cat(" Diagnostic: argmax distribution =", paste(names(table(max_proba_col)), table(max_proba_col), sep=":", collapse=", "), "\n")
    }
  } else {
    cat(" QA1: No point 'dredging' pre-smoothing\n")
  }
  ...
  if (n_smooth_dredge > 0) {
    ...
    if (q["50%"] >= 0.5) {
      cat(sprintf(" QA2 (post-smooth): median p_dredge = %.3f (≥0.5)\n", q["50%"]))
    } else if (q["50%"] >= 0.3) {
      cat(sprintf(" QA2 (post-smooth): median p_dredge = %.3f (low, expected ≥0.5)\n", q["50%"]))
      cat(sprintf(" Quantiles: 10%%=%.3f, 25%%=%.3f, 50%%=%.3f, 75%%=%.3f, 90%%=%.3f\n",
                  q["10%"], q["25%"], q["50%"], q["75%"], q["90%"]))
    } else {
      cat(sprintf(" QA2 (post-smooth): median p_dredge = %.3f (tres low!)\n", q["50%"]))
      cat(sprintf(" Quantiles: 10%%=%.3f, 25%%=%.3f, 50%%=%.3f, 75%%=%.3f, 90%%=%.3f\n",
                  q["10%"], q["25%"], q["50%"], q["75%"], q["90%"]))
      cat(" The smoothing a peut-etre too etendu the segments dredging\n")
    }
  } else {
    cat(" QA2: No point 'dredging' post-smoothing\n")
  }
...
resGS <- checkpoint(
  ...
  {
    cat(" Grid-search (poids score dredging) - CORRIGE FOR EVITER DATA LEAKAGE\n")
    ...
    cat(" Validation croisee LOYO stricte:", cv_folds, "folds (1 year = 1 fold)\n")
    ...
    # === COMPOSITE DREDGING SCORE: 5 signals → cv.glmnet (ridge) → LOYO → Youden J ===
    ...
    if (is.null(gmm_info)) {
      stop(" ERROR: Informations of mapping GMM missing (attribut gmm_info)")
    }
    ...
    if (!dredge_col %in% names(ais)) {
      stop(sprintf(" ERROR: Column %s missing (dredge_idx=%d)", dredge_col, dredge_idx))
    }
    
    ais[, p_dredge := get(dredge_col)]
    cat(sprintf(" Probabilite dredging: %s (composante %d, speed %.2f kn, K=%d)\n", 
    ...
    cat(" Verification: Exactly 5 signals required for the composite score\n")
    
    # FIX: Replace remaining NAs in speed_bin and speed_norm before contoion
    ais[is.na(speed_bin),  speed_bin  := 0L]
    ais[is.na(speed_norm), speed_norm := 0]
    ...
    # DIAGNOSTIC: Check for not-finite values in predictors
    msg <- colSums(!is.finite(as.matrix(predictors)))
    cat(" Not-finis par column :", paste(names(msg), msg, sep=":", collapse=", "), "\n")
    
    # FIX: Replace not-finite values with 0
    predictors <- predictors[, lapply(.SD, function(v) replace(v, !is.finite(v), 0))]
    cat(" Valeurs not-finies remplacees par 0\n")
    
    # CRITICAL FIX: Remove NAs from target (DATA PRESERVATION)
    cat(" Nettoyage the NA in the variable cible...\n")
    ...
    if (any(is.na(target[keep_lbl]))) {
      stop("ERROR FATALE : NA detectes in target after filtrage !")
    }
    
    # Convert to 0/1 integer (glmnet prefers numeric) - for training only
    target_train <- as.integer(target[keep_lbl])  # 1 = dredging, 0 = not-dredging
    ...
    cat(sprintf(" Data preservees: %d points for entrainement, %d points for prediction complete\n",
                length(train_indices), n_before))
    
    # FIX: Handle NA values in predictors
    # glmnet does not accept NA values
    cat(" Verification the missing values in the predictors...\n")
    na_counts <- colSums(is.na(predictors))
    cat(" Valeurs NA par predicteur:", paste(names(na_counts), na_counts, sep=":", collapse=", "), "\n")
    
    # DIAGNOSTIC: Verify p_dredge mapping
    cat(" mapping verification p_dredge:\n")
    cat(" Columns p disponibles:", paste(intersect(paste0("p",1:5), names(ais)), collapse=", "), "\n")
    cat(" Column p_dredge created:", if("p_dredge" %in% names(ais)) "OUI" else "NOT", "\n")
    ...
    cat(" Valeurs NA remplacees par 0\n")
    ...
    cat(" Predictors utilises:", paste(names(predictors), collapse = ", "), "\n")
    ...
    if (is_dry) {
      ...
      if (dredging_count < 10) {
        stop(sprintf("ERROR FATALE : Only %d points 'dredging' detectes (< 10), echantillon too imbalance for the dry-run !", dredging_count))
      }
      if (non_dredging_count < 50) {
        stop(sprintf("ERROR FATALE : Only %d points 'not-dredging' detectes (< 50), echantillon too imbalance for the dry-run !", non_dredging_count))
      }
    } else {
      if (dredging_count < 100) {
        stop(sprintf("ERROR FATALE : Only %d points 'dredging' detectes (< 100), impossible of faire une CV valide !", dredging_count))
      }
      if (non_dredging_count < 1000) {
        stop(sprintf("ERROR FATALE : Only %d points 'not-dredging' detectes (< 1000), impossible of faire une CV valide !", non_dredging_count))
      }
    }
    
    # FIX P2.1: Long-format summary instead of 3D table (avoids memory explosion)
    cat(" Distribution the classes par year/vessel (top 20):\n")
    ...
    if (length(unique(target_train)) < 2) {
      stop(sprintf("ERROR FATALE : Dataset single-class ! Only %d classe(s) detectee(s), impossible of faire une CV valide.\n Distribution: %s", 
                   length(unique(target_train)), paste(table(target_train), collapse=", ")))
    } else {
      # FIX 1: Check the packages offline before CV
      cat(" Verification the packages required...\n")
      if (!requireNamespace("glmnet", quietly = TRUE)) {
        stop(" Package 'glmnet' absent - loaded-the via module ou $R_LIBS_USER")
      }
      if (!requireNamespace("doParallel", quietly = TRUE)) {
        stop(" Package 'doParallel' absent - loaded-the via module ou $R_LIBS_USER")
      }
        library(glmnet)
      library(doParallel)
      
      # FIX 6: glmnet parallelism without data.table conflict
      cat(" Configuration of parallelism glmnet...\n")
      ...
      cat(" parallelism configured for glmnet (", min(12, parallel::detectCores() - 2), " cores)\n")
...
if (length(missing) > 0) {
  cat(" Add the columns missing:", paste(missing, collapse=", "), "\n")
  predictors_train[, (missing) := 0]
}

# FIX 9: Convert to sparse matrix to save memory
cat(" Contoion en matrice creuse for economiser the memory...\n")
if (!requireNamespace("Matrix", quietly = TRUE)) {
  stop(" Package 'Matrix' absent - loaded-the via module ou $R_LIBS_USER")
}
library(Matrix)
...
cat(sprintf(" Matrice creuse globale created: %.1f MB\n", 
...
cat(sprintf(" Matrice creuse d'entrainement: %.1f MB (vs %.1f MB dense)\n", 
...
auc_fold <- numeric(max(ais_train$fold))
      ...
      for (k in unique(ais_train$fold)) {
        ...
        if (sum(complete_train, na.rm = TRUE) < min_train || sum(complete_test, na.rm = TRUE) < min_test) {
          stop(sprintf("ERROR FATALE : Fold %d too small after filtrage NA (train: %d < %d, test: %d < %d)", 
        ...
        if (!train_has_2) {
          stop(sprintf("ERROR FATALE : Fold LOYO %d - train single-class (impossible of fitter)", k))
        }

        # DIAGNOSTIC: Class counts per fold
        cat(sprintf(" Dredging (train/test): %d/%d, Not-dredging (train/test): %d/%d\n",
          sum(Ytrain[complete_train], na.rm = TRUE), sum(Ytest[complete_test], na.rm = TRUE),
          sum(!Ytrain[complete_train], na.rm = TRUE), sum(!Ytest[complete_test], na.rm = TRUE)))

        if (!test_has_2) {
          cat(" Test single-class → AUC = NA for this fold\n")
        }
        ...
        tryCatch({
          ...
          # FIX P2.4: Fonction of stratification qui stratifie vraiment
          ...
          if (!folds_ok) cat(" Folds internal imbalanced → fallback deviance\n")
      ...
      if (all(is.na(auc_fold))) {
        stop("Tous the folds ont failed : check the presence of valeurs not finies in the predictors.")
      }
      ...
      if (length(valid_auc) > 0 && length(best_models) > 0) {
        ...
        # FIX 8: More robust check for not-zero coefficients
        if (total_weight == 0 || length(feature_coefs) == 0) {
          stop("ERROR FATALE : Regression without poids not nuls - Check the predictors")
        }
        ...
        cat(" Poids of composite score saved in drag_score_weights.csv\n")
        ...
        # Application of threshold optimal on toutes the rows
        ...
        gc()
        cat(" Memory nettoyee after CV\n")
      } else {
        stop(sprintf("ERROR FATALE : No fold valide for the CV !\n Diagnostic: No fold valide found\n Summary the AUC par fold: %s\n Nombre of modeles stockes: %d\n Check the distribution the classes par year.", 
                     paste(sprintf("%.3f", auc_fold), collapse=", "), length(best_models)))
      }
    }
    
    cat(" Grid-search completed - AUC honnete calcule\n")
...
cat(" Save the probabilites GMM for quality verification...\n")

# FIX: Gestion of the column ssvid vs MMSI
if ("ssvid" %in% names(ais)) {
  id_col <- "ssvid"
  ...
  warning(" No column d'identifiant found (ssvid/MMSI/Vessel) - utilisation of the premiere column")
  id_col <- names(ais)[1]
}

# FIX 2: QA robuste - ne garder que the p* existantes (dynamique selon K)
gmm_info <- attr(ais, "gmm_info")
if (!is.null(gmm_info)) {
...
cat(" Probabilites GMM saved in GMM_probabilities_QA.rds (columns p:", paste(qa_p, collapse=", "), ")\n")

  # Safe purge of helper columns based on current GMM mapping
# PATCH: Keep dredge_lisse, norm_course, norm_acc, Course_change and Accel for QC
  ...
  if (!is.null(gmm_info)) {
    dredge_idx <- gmm_info$dredge_idx
    dredge_col <- paste0("p", dredge_idx + 1)  # +1 because p1 = stops
    cat(" Conservation of the column dredging:", dredge_col, "\n")
  ...
  # Include additional helper columns while filtering non-existing names
...
if (length(keep)) {
  ais[, (keep) := NULL]
  cat(" Columns temporaires removed:", paste(keep, collapse=", "), "\n")
} else {
  cat(" No column temporaire a supprimer\n")
}
...
if (length(todrop)) {
  ais[, (todrop) := NULL]
  cat(" Columns of stats removed:", paste(todrop, collapse=", "), "\n")
} else {
  cat(" No column of stats a supprimer\n")
}
...
cat(" Verrou global: No NA in behavior_smooth - Pipeline hermetique\n")
## ─────────────────────────────────────────────────────────
...
if ("p_dredge_qa" %in% names(ais)) {
  ...
  cat(" Column QA p_dredge_qa removed\n")
}

# EARLY EXIT IN DRY-RUN MODE
if (is_dry) {
  cat("\n === DRY-RUN COMPLETED successfully ===\n")
  cat(" Test complet jusqu'to grid-search - No file lourd save\n")
  cat(" Pipeline ready for the run complet\n")
