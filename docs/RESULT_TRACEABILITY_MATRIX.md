...
| R1 | Vessel split by identifier | `pipeline/step1/step1_split_vessels.R` via `pipeline/step1/step1_split_vessels.sh` | Raw AIS tracks, `core_window.yaml` | Per-vessel RDS files |
...
| R5 | SAR and f_i grids | `pipeline/step5/step5_make_tiles.R`, `pipeline/step5/step5_tile_worker.R`, `pipeline/step5/step5_merge_tiles.R` | Step4 outputs, `config/fi_parameters.yaml`, Longhurst, land mask | `sar_*.parquet`, `fi_grid_*.parquet`, `fi_grid_*.tif` |
