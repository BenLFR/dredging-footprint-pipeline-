...
# Map deploy vars -> runtime pipeline vars forwarded to every SLURM job
# These can be overridden in config.env or environment before calling this script.
...
# Common env var string forwarded to every sbatch --export
...
# Canonical script locations (subdirectory layout for pub/v1.0-clean)
STEP0_SCRIPT="${STEP0_SCRIPT:-pipeline/step0/step0_window_select.sh}"
STEP1_SCRIPT="${STEP1_SCRIPT:-pipeline/step1/step1_split_vessels.sh}"
STEP2_SCRIPT="${STEP2_SCRIPT:-pipeline/step2/step2_process_array.sh}"
STEP3_SCRIPT="${STEP3_SCRIPT:-pipeline/step3/step3_merge.sh}"
STEP4_SCRIPT="${STEP4_SCRIPT:-pipeline/step4/step4_add_lithology.sh}"
STEP5B_SCRIPT="${STEP5B_SCRIPT:-pipeline/step5/step5_tile_job.sh}"
STEP5C_SCRIPT="${STEP5C_SCRIPT:-pipeline/step5/step5_merge_slurm.sh}"
STEP6_SCRIPT="${STEP6_SCRIPT:-pipeline/step6/step6_calculate_cri.sh}"
STEP7_SCRIPT="${STEP7_SCRIPT:-pipeline/step7/step7_export_jdredge.sh}"
...
if (( from_step <= 0 )); then
  job0="$(submit_remote_script "${STEP0_SCRIPT}" --export "${COMMON_EXPORTS}")"
...
if (( from_step <= 1 )); then
  ...
  job1="$(submit_remote_script "${STEP1_SCRIPT}" "${dep_args[@]}" --export "${COMMON_EXPORTS}")"
...
if (( from_step <= 2 )); then
  ...
  dep_args=()
  [[ -n "${prev_job}" ]] && dep_args=(--dependency "afterok:${prev_job}")
  job2="$(submit_remote_script "${STEP2_SCRIPT}" "${dep_args[@]}" --array "${step2_array_spec}" --export "${COMMON_EXPORTS},SPLIT_JOB_ID=${step2_split_job_id}")"
...
if (( from_step <= 3 )); then
  ...
  dep_args=()
  [[ -n "${prev_job}" ]] && dep_args=(--dependency "afterok:${prev_job}")
  job3="$(submit_remote_script "${STEP3_SCRIPT}" "${dep_args[@]}" --export "${COMMON_EXPORTS},SPLIT_JOB_ID=${step3_split_job_id},RESULTS_DIR=${step3_results_dir}")"
...
if (( from_step <= 4 )); then
  dep_args=()
  [[ -n "${prev_job}" ]] && dep_args=(--dependency "afterok:${prev_job}")
  job4="$(submit_remote_script "${STEP4_SCRIPT}" "${dep_args[@]}" --export "${COMMON_EXPORTS}")"
...
if (( from_step <= 5 )); then
  dep_args=()
  [[ -n "${prev_job}" ]] && dep_args=(--dependency "afterok:${prev_job}")
  step5a_wrap="Rscript $(printf '%q' "${RUNTIME_PIPELINE_DIR}/step5/step5_make_tiles.R")"
  job5a="$(submit_remote_wrap "${step5a_wrap}" "${dep_args[@]}" --job-name step5_make_tiles --export "${COMMON_EXPORTS}")"
  record_job step5a "${job5a}"

  job5b="$(submit_remote_script "${STEP5B_SCRIPT}" --dependency "afterok:${job5a}" --export "${COMMON_EXPORTS}")"
  record_job step5b "${job5b}"

  job5c="$(submit_remote_script "${STEP5C_SCRIPT}" --dependency "afterok:${job5b}" --export "${COMMON_EXPORTS}")"
...
if (( from_step <= 6 )); then
  dep_args=()
  [[ -n "${prev_job}" ]] && dep_args=(--dependency "afterok:${prev_job}")
  job6="$(submit_remote_script "${STEP6_SCRIPT}" "${dep_args[@]}" --export "${COMMON_EXPORTS}")"
...
if (( from_step <= 7 )); then
  dep_args=()
  [[ -n "${prev_job}" ]] && dep_args=(--dependency "afterok:${prev_job}")
  job7="$(submit_remote_script "${STEP7_SCRIPT}" "${dep_args[@]}" --export "${COMMON_EXPORTS}")"
