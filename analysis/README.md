...
otherwise under `configuration/` and then `config/`. The expected parameter
file is `fi_parameters_with_freshness.yaml`.
...
| `sensitivity_oat_analysis.R` | Runs one OAT task from the sensitivity manifest and recomputes `C_ri` on the full `fi_grid` when available. | sensitivity manifest, `fi_parameters_with_freshness.yaml`, `fi_grid_*` | `OUTPUT_DIR/sensitivity/oat_results.csv` and/or `oat_task_XX.csv` | `Rscript analysis/sensitivity_oat_analysis.R 28` |
...
| `diagnose_sobol_convergence.R` | Re-runs the synthetic Sobol proxy at several sample sizes to check whether rankings and `ST` values stabilize. | `fi_parameters_with_freshness.yaml` if available | `OUTPUT_DIR/diagnostics/sobol_convergence_*.csv`, `sobol_convergence_plot.png` | `Rscript analysis/diagnose_sobol_convergence.R 250,500,1000` |
...
| `check_fi_grid_consistency.R` | Audits a published `fi_grid` against the step 5 formula and core schema assumptions. | latest `fi_grid_*`, optional `fi_parameters_with_freshness.yaml` | `OUTPUT_DIR/diagnostics/fi_grid_consistency_checks.csv`, `fi_grid_consistency_summary.txt` | `Rscript analysis/check_fi_grid_consistency.R` |
