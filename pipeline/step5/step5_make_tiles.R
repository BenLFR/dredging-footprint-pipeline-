...
cat(" Generation the tiles mondiales for Step-5\n")

# Tiling parameters (from constants.R)
cell_km   <- CELL_KM   # taille of tile en km
...
# Affichage robuste of l'emprise
bb <- st_bbox(world_bb)
cat(" Emprise mondiale :\n")
cat(" - X: ", format(bb["xmin"], scientific=FALSE), " a ", 
    format(bb["xmax"], scientific=FALSE), " m\n")
cat(" - Y: ", format(bb["ymin"], scientific=FALSE), " a ", 
...
cat(" Generation of the grid of tiles...\n")
cat(" - Nombre of columns (X) :", nx, "\n")
cat(" - Nombre of lines (Y) :", ny, "\n")
cat(" - Total expected :", nx * ny, "tiles\n")

# Generate grid with fixed dimensions
tiles_sf <- sf::st_make_grid(
  offset   = c(WORLD_XMIN, WORLD_YMIN),
  cellsize = cell_km * 1000,
  n        = c(nx, ny),        # ← Force exactly 35×18
  crs      = crs_eq,
  what     = "polygons"
) %>%
  sf::st_sf()
...
cat(" - Lines generated :", n_rows, "(expected:", ny, ")\n")
...
cat(" - Taille par tile :", cell_km, "×", cell_km, "km\n")
cat(" - Surface totale :", format(surface_km2, big.mark=","), "km2\n")
...
if(length(missing_ids) > 0) {
  cat(" IDs missing :", paste(missing_ids, collapse=", "), "\n")
} else {
  cat(" Tous the IDs of 1 a", n_tiles, "are presents\n")
}

# Save with nom dynamique
output_file <- sprintf("~/scratch/output_V6/tiles_%dkm.gpkg", cell_km)
cat(" Save in :", output_file, "\n")
...
if(file.exists(output_file)) {
  ...
  stop(" Error lors of the creation of file")
}
...
cat("\n Next steps :\n")
cat(" 1. Check the nombre of tiles : ogrinfo -ro -so", output_file, "tiles | grep 'Feature Count'\n")
cat(" 2. Run l'array-job : sbatch --array=1-", n_tiles, " step5_tile_job.sh\n")
cat(" 3. Monitorer : squeue -u $USER | grep step5_tile\n")

cat("\n Generation the tiles completed !\n") 
