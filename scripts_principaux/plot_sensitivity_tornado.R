...
#
# Usage:
#   Rscript scripts_principaux/plot_sensitivity_tornado.R
# ============================================================================
...
# ── 1. Load OAT results ───────────────────────────────────────────────────────
# Search scratch first (GRIT ~/scratch), then local output_V6/
oat_candidates <- c(
  file.path(path.expand("~"), "scratch", "output_V6", "sensitivity", "oat_results.csv"),
  "output_V6/sensitivity/oat_results.csv"
)
oat_path <- oat_candidates[file.exists(oat_candidates)][1]
if (is.na(oat_path)) {
  stop(paste(
    "oat_results.csv not found in ~/scratch/output_V6/sensitivity/ or output_V6/sensitivity/.",
    "Run sensitivity_oat_analysis.R for all 55 task IDs first:",
    "  sbatch --partition=emlab_nodes scripts_cluster/submit_sensitivity_array.sh"
  ))
}
...
p <- ggplot(plot_dt, aes(x = pct, y = param_label, fill = direction)) +
  ...
  labs(
    ...
    subtitle = paste("Parameters from fi_parameters.yaml | ±50% perturbation from defaults",
