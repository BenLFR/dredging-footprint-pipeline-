...
mean_valid_endpoint_pl <- function(pl_start, pl_end) {
  n_valid <- as.integer(!is.na(pl_start)) + as.integer(!is.na(pl_end))
  out <- rep(NA_real_, length(pl_start))
  keep <- n_valid > 0L

  if (any(keep)) {
    pl_start0 <- pl_start
    pl_end0 <- pl_end
    pl_start0[is.na(pl_start0)] <- 0
    pl_end0[is.na(pl_end0)] <- 0
    out[keep] <- (pl_start0[keep] + pl_end0[keep]) / n_valid[keep]
  }

  out
}

empty_carbon_segments_sf <- function(target_crs) {
  st_sf(
    data.table(sub_seg_id = character(), pl_seg = numeric()),
    geometry = st_sfc(crs = target_crs)
  )
}

build_carbon_segments_sf <- function(dredge, target_crs) {
  if (!nrow(dredge)) {
    return(empty_carbon_segments_sf(target_crs))
  }

  seg_dt <- copy(dredge)
  setorderv(seg_dt, c("sub_seg_id", "Timestamp"))
  seg_dt[, valid_pl := fifelse(use_for_carbon == TRUE & is.finite(pl_base), pl_base, NA_real_)]
  ...
  carbon_dt <- seg_dt[
    ,
    {
      ...
      next_pl <- shift(valid_pl, type = "lead")
      .(
        x0 = X,
        y0 = Y,
        x1 = next_x,
        y1 = next_y,
        pl_seg = mean_valid_endpoint_pl(valid_pl, next_pl)
      )
    },
    by = sub_seg_id
  ]
  ...
  st_sf(carbon_dt[, .(sub_seg_id, pl_seg)], geometry = carbon_geom)
}

accumulate_segment_pl_to_grid <- function(carbon_segments_sf, grid1km) {
  if (!nrow(carbon_segments_sf)) {
    return(data.table(grid_id = integer(), n_with_pl = integer(), pl_sum = numeric()))
  }

  pl_res <- data.table(grid_id = integer(), n_with_pl = integer(), pl_sum = numeric())

  for (i in seq_len(nrow(carbon_segments_sf))) {
    cand <- sf::st_intersects(carbon_segments_sf[i, ], grid1km, sparse = FALSE)[1, ]
    if (!any(cand)) next

    inter <- suppressWarnings(sf::st_intersection(carbon_segments_sf[i, ], grid1km[cand, ]))
    if (!nrow(inter)) next

    inter_len <- as.numeric(st_length(inter))
    grid_ids <- unique(inter$grid_id[inter_len > 0])
    if (!length(grid_ids)) next

    pl_i <- data.table(
      grid_id = grid_ids,
      n_with_pl = 1L,
      pl_sum = carbon_segments_sf$pl_seg[i]
    )

    pl_res[pl_i, on = "grid_id", `:=`(
      n_with_pl = ifelse(is.na(n_with_pl), i.n_with_pl, n_with_pl + i.n_with_pl),
      pl_sum = ifelse(is.na(pl_sum), i.pl_sum, pl_sum + i.pl_sum)
    )]
    new_rows <- pl_i[!pl_res, on = "grid_id"]
    if (nrow(new_rows) > 0) {
      pl_res <- rbindlist(list(pl_res, new_rows), use.names = TRUE)
    }
  }

  setkey(pl_res, grid_id)
  pl_res
}
...
# Recompute pl_eff defensively before building segment-level carbon support.
dredge[, `:=`(
  dz1 = pmin(dredging_depth_m, SURF_HORIZON),
  dz2 = pmin(pmax(0, dredging_depth_m - SURF_HORIZON), DEEP_HORIZON)  # CAPPED
)]
dredge[, pl_eff := (pl_base * dz1 + FACTOR_DEEP * pl_base * dz2) / pmax(dredging_depth_m, 1e-6)]
carbon_segments_sf <- build_carbon_segments_sf(dredge, target_crs)
cat("ℹ️  Segments support carbone :", nrow(carbon_segments_sf), "\n")
...
## 7.  Intersections lignes×grille (SAR) + support segmentaire pour p_l --------
...
# Raw p_l per cell from qualifying trajectory segments.
# Each consecutive segment propagates the mean of valid endpoint pl_base values
# to every crossed cell. Depth-weighting remains deferred to the merge step.
pl_cell <- accumulate_segment_pl_to_grid(carbon_segments_sf, grid1km)
