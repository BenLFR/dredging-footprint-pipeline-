#!/bin/bash
# Script de vérification de la configuration GRIT pour Step 0
...
echo "2️⃣  Vérification structure répertoires..."
...
echo "3️⃣  Vérification scripts Step 0..."
ssh -F "$SSH_CONFIG" "$REMOTE" "
    echo '   📄 Scripts:'
...
echo "4️⃣  Vérification données AIS..."
ssh -F "$SSH_CONFIG" "$REMOTE" "
    if [ -f ~/scratch/AIS_data/benjamin2.csv ]; then
        ...
        echo '   ⚠️  Utilisez: scp -F ~/.ssh/config_grit AIS_with_lithology_clean.csv grit:~/scratch/AIS_data/benjamin2.csv'
    fi
"
echo ""

echo "5️⃣  Vérification R..."
ssh -F "$SSH_CONFIG" "$REMOTE" "
    if command -v R &>/dev/null; then
        R --version | head -1
        echo '   ✅ R disponible'
    else
        echo '   ⚠️  R non trouvé dans PATH - vérifier les modules'
    fi
"
echo ""

echo "6️⃣  Vérification librairie R..."
ssh -F "$SSH_CONFIG" "$REMOTE" "
    if [ -d ~/R/library ]; then
        count=\$(ls ~/R/library 2>/dev/null | wc -l)
        echo \"   ✅ ~/R/library existe (\${count} packages)\"
    else
        echo '   ⚠️  ~/R/library non trouvé - créer avec: mkdir -p ~/R/library'
    fi
"
echo ""
...
ssh -F "$SSH_CONFIG" "$REMOTE" "
    ...
    if \$all_ok; then
        ...
        echo '⚠️  Configuration incomplète - Voir détails ci-dessus'
    fi
"
echo ""
