...
snap_cells2 <- function(DT, xcol="X", ycol="Y"){
  x <- as.numeric(DT[[xcol]]); y <- as.numeric(DT[[ycol]])
...
if(is_subtile){
  ...
  cat(sprintf("🔧 Sous-tuile %d_%d — bbox : (%.0f,%.0f)-(%.0f,%.0f)\n",
...
cat("🔧  Paramètres (pour info) : alpha_dep=", alpha_dep,
...
lithology_files <- list.files("~/scratch/output_V6/",
                              pattern="AIS_with_lithology_clean_.*\\.rds$",
                              full.names=TRUE)
if(!length(lithology_files))
  stop("❌ Aucun fichier AIS_with_lithology_clean_* trouvé.")

dt_lithology <- readRDS(max(lithology_files))
dredge_raw   <- dt_lithology[Dragage_flag==1 & !is.na(Lon) & !is.na(Lat)]
if(!nrow(dredge_raw)){ cat("⚠️  Aucun ping de dragage — sortie propre\n"); quit("no") }
...
if(!nrow(dredge_sf)){
  ...
  out <- if(is_subtile)
           sprintf(file.path(Sys.getenv("OUT_DIR","~/scratch/output_V6"), "sar_%d_sub%d.parquet"),tile_id,sub_id)
         else sprintf(file.path(Sys.getenv("OUT_DIR","~/scratch/output_V6"), "sar_%03d.parquet"),tile_id)
...
dredge <- as.data.table(dredge_sf); dredge[,geometry:=NULL]

# [CHANGEMENT] Harmonise l’identifiant navire
id_cols <- c("ssvid","SSVID","mmsi","MMSI","vessel_id","VESSEL_ID","Navire")
for (c in id_cols) if (c %in% names(dredge)) { setnames(dredge, c, "ssvid", skip_absent=TRUE); break }
...
if(!is_subtile){
  dredge[,`:=`(
    ...
    dz2 = pmax(0, dredging_depth_m - SURF_HORIZON),
    pl_eff = (pl_base*dz1 + FACTOR_DEEP*pl_base*dz2) / pmax(dredging_depth_m, 1e-6)
  )]
  detail_cols <- c("Timestamp","Lon","Lat","X","Y","col","row","grid_id","ssvid",
...
if(nrow(lines_sf) == 0){
  ...
  arrow::write_parquet(data.table(grid_id=integer(), sum_dw=numeric(), sum_dw_pd=numeric(),
...
out <- if(is_subtile) {
         ...
         sprintf(file.path(Sys.getenv("OUT_DIR","~/scratch/output_V6"), "sar_%03d.parquet"),tile_id)
       }
